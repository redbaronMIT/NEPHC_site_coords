...
<div class="tm_pb_post_meta"><?php
	...
	if ( 'on' === $this->_var( 'show_date' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				esc_html__( '%s', 'navyblue' ),
				'<strong class="published">' . esc_html( get_the_date( $this->_var( 'meta_date' ) ) ) . '</strong>'
			)
		);
	}
	...
	if ( 'on' === $this->_var( 'show_author' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				'<span>' .  esc_html__( 'Posted by %s', 'navyblue' ) . '</span>',
	...
	if ( 'on' === $this->_var( 'show_comments' ) ) {
		...
		printf(
			esc_html(
				_nx( '1 Comment', '%s Comments', get_comments_number(), 'number of comments', 'navyblue' )
			),
