 ...
 * @return array
 */
function navyblue_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! navyblue_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'navyblue_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'navyblue_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'navyblue' ) . $network['name'],
	...
	if( $meta === 'single_post_share_buttons' ){
		$share_label = get_theme_mod( 'single_post_share_label', navyblue_theme()->customizer->get_default( 'single_post_share_label' ) );
