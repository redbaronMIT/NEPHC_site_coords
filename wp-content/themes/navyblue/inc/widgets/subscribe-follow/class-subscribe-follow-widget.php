...
class NavyBlue_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'navyblue' );
		$this->widget_id          = 'navyblue_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'navyblue' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'navyblue' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Sign up and get first-hand', 'navyblue' ),
				'label' => esc_html__( 'Subscribe Title', 'navyblue' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'value' => esc_html__( 'discounts & dancing tips!', 'navyblue' ),
				'label' => esc_html__( 'Subscribe text message', 'navyblue' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Your e-mail address', 'navyblue' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'navyblue' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'navyblue' ),
				'label' => esc_html__( 'Subscribe submit label', 'navyblue' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'navyblue' ),
				'label' => esc_html__( 'Subscribe success', 'navyblue' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'navyblue' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'navyblue' ),
				'label' => esc_html__( 'Follow Title', 'navyblue' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'navyblue' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'navyblue' ),
			...
			'background_color_subscribe' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Subscribe Background Color', 'navyblue' ),
				'master' => 'background_image',
			),
			'background_color_follow' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Follow Background Color', 'navyblue' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_navyblue_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_navyblue_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'navyblue' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'navyblue' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'navyblue' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'navyblue' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'navyblue' );
		}
	...
	public function get_social_nav() {
		return navyblue_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = navyblue_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'navyblue_subscribe' ) ) {
...
add_action( 'widgets_init', 'navyblue_register_subscribe_follow_widgets' );
function navyblue_register_subscribe_follow_widgets() {
	register_widget( 'NavyBlue_Subscribe_Follow_Widget' );
}
