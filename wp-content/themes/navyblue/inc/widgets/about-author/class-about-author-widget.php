 ...
 * @package NavyBlue
 */
if ( ! class_exists( 'NavyBlue_About_Author_Widget' ) ) {

	class NavyBlue_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'navyblue widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'navyblue' );
			$this->widget_id          = 'navyblue_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'navyblue' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'navyblue' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'navyblue' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'navyblue' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'navyblue' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'navyblue' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'navyblue' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'navyblue' ),
					'label' => esc_html__( 'Link label', 'navyblue' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'navyblue' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'navyblue-author-avatar' );
...
add_action( 'widgets_init', 'navyblue_register_about_author_widgets' );
function navyblue_register_about_author_widgets() {
	register_widget( 'NavyBlue_About_Author_Widget' );
}
