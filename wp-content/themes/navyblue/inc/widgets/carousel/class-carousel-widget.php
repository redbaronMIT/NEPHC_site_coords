 ...
 * @package NavyBlue
 */
if ( ! class_exists( 'NavyBlue_Carousel_Widget' ) ) {

	class NavyBlue_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description	= esc_html__( 'Display a list of your posts on your site.', 'navyblue' );
			$this->widget_id			= 'navyblue_widget_carousel';
			$this->widget_name			= esc_html__( 'Carousel', 'navyblue' );
			$this->utility				= navyblue_utility()->utility;
			$this->settings				= array(
				'title' => array(
					'type'				=> 'text',
					'value'				=> '',
				...
				'terms_type' => array(
					...
					'options' => array(
						'category' => array(
							'label'		=> esc_html__( 'Category', 'navyblue' ),
							'slave'		=> 'terms_type_category',
						),
						'post_tag' => array(
							'label'		=> esc_html__( 'Tag', 'navyblue' ),
							'slave'		=> 'terms_type_post_tag',
						),
					),
					'label' => esc_html__( 'Choose taxonomy type', 'navyblue' ),
				),
				'categories' => array(
					'type'				=> 'select',
					'size'				=> 1,
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select category', 'navyblue' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select category', 'navyblue' ),
				...
				'tags' => array(
					...
					'options'			=> false,
					'label'				=> esc_html__( 'Select tags', 'navyblue' ),
					'multiple'			=> true,
					'placeholder'		=> esc_html__( 'Select tags', 'navyblue' ),
				...
				'posts_per_page' => array(
					...
					'max_value'			=> 20,
					'min_value'			=> 1,
					'label'				=> esc_html__( 'Posts count', 'navyblue' ),
				),
				'post_title' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display title', 'navyblue' ),
				),
				'content' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display content', 'navyblue' ),
				),
				'more_button' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display more button', 'navyblue' ),
					'toggle'			=> array(
						'true_toggle'	=> esc_html__( 'On', 'navyblue' ),
						'false_toggle'	=> esc_html__( 'Off', 'navyblue' ),
				...
				'more_button_text' => array(
					'type'				=> 'text',
					'value'				=> esc_html__( 'Read more', 'navyblue' ),
					'label'				=> esc_html__( 'More button text', 'navyblue' ),
				...
				'trim_words' => array(
					...
					'min_value'			=> 1,
					'step_value'		=> 1,
					'label'				=> esc_html__( 'Content words trimmed count', 'navyblue' ),
				),
				'slides_per_view' => array(
					...
					'min_value'			=> 1,
					'value'				=> 5,
					'label'				=> esc_html__( 'Number of slides per view', 'navyblue' ),
				),
				'slides_per_group' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Number slides per group', 'navyblue' ),
				),
				'slides_per_column' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Multi Row Slides Layout', 'navyblue' ),
				),
				'space_between_slides' => array(
					...
					'min_value'			=> 0,
					'value'				=> 30,
					'label'				=> esc_html__( 'Width of the space between slides(px)', 'navyblue' ),
				),
				'duration_speed' => array(
					...
					'min_value'			=> 100,
					'value'				=> 500,
					'label'				=> esc_html__( 'Duration of transition between slides (ms)', 'navyblue' ),
				),
				'navigation' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider navigation', 'navyblue' ),
				),
				'pagination' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider pagination', 'navyblue' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', navyblue_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="navyblue-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'navyblue' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'navyblue_carousel_default_query_args', array(
				'post_type'			=> 'post',
				'orderby'			=> 'date',
				'order'				=> 'DESC',
				'posts_per_page'	=> -1,
		...
		public function get_carousel_loop( $posts_query ) {
			...
			if ( ! $view_dir ) {
				return '<h5>' . esc_html__( 'View file not found', 'navyblue' ) . '</h5>';
			}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				...
				$image = $this->utility->media->get_image( array(
					'size'				=> 'navyblue-thumb-560-350',
					'mobile_size'		=> 'navyblue-thumb-s',
	...
	add_action( 'widgets_init', 'navyblue_register_carosel_widgets' );
	function navyblue_register_carosel_widgets() {
		register_widget( 'NavyBlue_Carousel_Widget' );
	}
}